...
- **Workbench + Card detail UX polish (shipped on v2 branch):** shared list UX now includes **selected / matching / set** enqueue flows from Explore with 5,000-cap warning parity, large-set confirm guardrails (including anti-double-click dialog protection), progressive list target pickers, and card-detail action declutter (inline share link, quieter metadata/chips, hover-only filter underlines, and delete moved into **More** menu).
