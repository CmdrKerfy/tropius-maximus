-- ============================================================
-- 044: camera_angle TEXT → JSONB[] with multi_select
-- ============================================================
-- Converts camera_angle from a single-value TEXT column to a
-- JSONB array column (matching art_style, pose, emotion, etc.)
-- so users can select multiple camera angles per card.
--
-- expand-contract alternative considered but single migration
-- accepted: 3-user app, single deploy target, negligible traffic.
-- ============================================================
  ...
  SELECT a2.card_id,
    COALESCE(
      (SELECT jsonb_agg(trimmed ORDER BY idx)
       FROM (
         SELECT DISTINCT ON (lower(trim(elem))) trim(elem) AS trimmed, idx
         FROM unnest(
           string_to_array(
             regexp_replace(
               regexp_replace(a2.camera_angle, '\s*[,;，；]\s*', '|', 'g'),
               '\|+', '|', 'g'
             ),
             '|'
           )
...
-- 5. Update get_form_options_db RPC: camera_angle from scalar DISTINCT to unnest
-- Must reproduce the full function since we're changing its body.
...
AS $$
  ...
  jnorm AS (
    SELECT
      ...
      -- camera_angle is now JSONB[] — normalize like other array columns
      CASE
        WHEN jsonb_typeof(COALESCE(camera_angle, '[]'::jsonb)) = 'array' THEN COALESCE(camera_angle, '[]'::jsonb)
        ELSE '[]'::jsonb
      END AS camera_angle
    FROM ann
  )
  SELECT jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      -- camera_angle: now JSONB[] — unnest like other array columns
      'camera_angle',
      COALESCE(
        (SELECT jsonb_agg(x ORDER BY x)
         FROM (
           SELECT DISTINCT elem AS x
           FROM jnorm j,
           LATERAL jsonb_array_elements_text(j.camera_angle) AS elem
           WHERE elem IS NOT NULL AND btrim(elem) <> ''
         ) d),
        '[]'::jsonb
      ),
