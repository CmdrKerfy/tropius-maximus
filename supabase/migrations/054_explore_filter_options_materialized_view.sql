-- Migration 054: Materialized view for Explore filter options
--
-- Replaces the expensive client-paged distinct scans (up to ~20 PostgREST
-- round-trips per page load) with a single <50ms lookup. The view is
-- refreshed after ingest, not on every page load.
--
-- The app reads all four source rows, passes them through the existing
-- mergeExploreFilterOptions(), and returns the merged payload. If the view
-- is empty (never refreshed), the app falls back to the client-paged path.

-- 1. Materialized view — one row per source, options as JSONB -----------------
